...
export function getConfectionReceipt({ company, confection }, isLastEl) {
    ...
    return (
      ...
      <div class=" h-full w-full flex justify-center items-center text center font-semibold">
        <div class="text-center flex justify-between flex-col h-full relative">
          ...
          <div class="text-left leading-[0.6rem]">
              ${gnCompany.address ?? ''}</br>
              EMAIL: <span class="tracking-tighter">${gnCompany?.email ?? '---'}</span> / TEL: ${
                gnCompany?.phone_nbr ?? '---'
              }</br>
              La Marque: ${confection?.final_product?.brand?.label ?? ''}</br>
              Produit: ${
              ...
              Station N°: 1557 Z // Exportateur N° ${gnCompany?.exporter_nbr ?? ''}</br>
              LOT N°: ${
              ...
              }A // Autorisation Saniitaire N°: MA FLC.60.36.14</br>
          </div>
          <div>
