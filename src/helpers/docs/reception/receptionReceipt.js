...
export function getReceptionReceipt(
  reception,
  lastIndex,
  company = companyStore.companies[0],
  internal = receptionStore.type.internal,
) {
  let tbody = ''
  ...
  Object.values(groupedPallets).map(
    (pallet) =>
      ...
      <tr class="border-b border-x [&>*]:py-1 [&>*]:border-black [&>*]:border-r">
        ...
        <td>${(pallet.total_gross_weight / pallet.total_nbr_boxes).toFixed(2)}</td>
      </tr>`),
  )
    ...
    <div class="grid grid-cols-2 text-[5px] p-3 font-medium">
      <div class="col-span-2 grid grid-cols-2">
        <div class="col-span-1">
          <h2 class="font-bold text-[8px]">${isGanFruits ? '' : company.name}</h2>
          <div class="flex flex-col text-[4px]"><span>${isGanFruits ? '' : company.address}</span></div>
        </div>
      ...
      <table class="text-center w-full mt-2 text-[3px]">
        <thead class="uppercase border border-black text-bold  bg-gray-50">
          <tr class="[&>*]:border-black [&>*]:py-1 [&>*]:border-r">
            ...
            <th scope="col">Code cultivation</th>
           <th scope="col">GGN</th>
            <th scope="col">Ferme</th>
            <th scope="col">Varieté</th>
            <th scope="col">Variété champ</th>
