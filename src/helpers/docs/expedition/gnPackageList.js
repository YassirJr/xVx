...
export function getGnPackageList(element, lastIndex) {
  const confections = element.confections
  const gnCompany = confections[0]?.company ? GAN_FRUITS_COMPANIES[confections[0]?.company] : ''
  let tableRows = ``
  let nbrBoxesTotal = 0
  ...
  uniqueConfections.forEach((confection, index) => {
    ...
    <th class="border-r text-[6px] border-b border-black py-0.5 font-normal">
      ${varietyName ?? ''} ${cultivation?.variety_field?.name ?? ''}
    </th>
    ...
    <div class="flex flex-col h-full">
      ...
      <div class="flex justify-between text-[6px] mb-2">
        <div class="grid grid-cols-2 gap-x-4">
          ...
          <div class="flex flex-col gap-y-1 justify-between text-center">
            ...
            <div class="p-0.5 font-semibold">${element.vehicle?.transporter?.name ?? ''}</div>
          </div>
        </div>
        <div class="grid grid-cols-2 gap-x-4">
          <div class="flex flex-col gap-y-1  text-center font-bold">
          ...
          <div class="flex flex-col gap-y-1 text-center">
            ...
            <div class="p-0.5 font-semibold">${element.vehicle?.registration_nbr}</div>
          </div>
        </div>
      </div>
      <!-- Table -->
          ...
          <tfoot>
            <tr>
              ...
              <th
                ...
                ${nbrBoxesTotal.toFixed(2)}
              </)th>
              <th
                class="border-b border-r border-black py-1 bg-gray-100">
                ${element.gross_weight.toFixed(2)}
              </th>
              <th class="border-b border-r border-black  py-1 bg-gray-100  ">
                ${element.net_weight.toFixed(2)}
              </th>
            </tr>
          </tfoot>
        </table>
      ...
      <div class="grid grid-cols-3 gap-x-8 text-[6px] mt-auto">
        ...
        <div class="text-center">
          ...
          <div>${gnCompany?.phone_nbr ? `Tel | ${gnCompany?.phone_nbr}` : ''}</div>
        </div>
